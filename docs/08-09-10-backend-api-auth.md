...
| Method | Path | Auth | Purpose · body | Success | Key errors |
|---|---|---|---|---|---|
| POST | `/geo/route` | bearer | Road directions · `pickup{lat,lng}, dropoff{lat,lng}`; validates the complete geometry against Bangladesh's boundary and returns the shortest domestic option plus valid domestic alternatives | 200 `{distanceKm,durationMin,path,alternatives}` | 422 `ROUTE_NOT_FOUND` · `OUTSIDE_SERVICE_AREA` · `DOMESTIC_ROUTE_NOT_FOUND` |
