...
export function RateTripCard({ tripCode, counterpartyName, existing }: RateTripCardProps) {
  ...
  async function submit() {
    ...
    try {
      ...
      toast.success(t('Thanks for your rating.'));
    } catch (thrown) {
      toast.error(getApiErrorMessage(thrown, t('Could not save your rating.')));
    } finally {
      setBusy(false);
    }
  }
  ...
  if (rating) {
    return (
      <Card className="mb-4 flex items-center justify-between gap-3 print:hidden">
        <p className="text-sm text-ink-500">{t('You rated {0}', firstName)}</p>
        <span className="flex text-marigold-500" aria-label={t('{0} out of 5 stars', rating.score)}>
  ...
  return (
    <Card className="mb-4 print:hidden">
      <h2 className="font-semibold">{t('How was your trip with {0}?', firstName)}</h2>
      <div className="mt-3 flex items-center gap-3">
        <div className="flex" role="radiogroup" aria-label={t('Rating')} onMouseLeave={() => setHover(0)}>
          {[1, 2, 3, 4, 5].map((value) => (
            <button
              key={value}
              type="button"
              ...
              aria-label={t('{0} star{1}', value, value > 1 ? 's' : '')}
      ...
      {score > 0 && (
        <div className="mt-3 space-y-3">
          <textarea
            ...
            maxLength={500}
            rows={2}
            placeholder={score <= 3 ? t('What could have gone better? (optional)') : t('Anything to add? (optional)')}
          ...
          />
          <Button loading={busy} onClick={() => void submit()}>{t('Submit rating')}</Button>
        </div>
      )}
    </Card>
  );
