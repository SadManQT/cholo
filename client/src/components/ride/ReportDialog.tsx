...
const CATEGORIES: Array<{ value: ReportCategory; label: string }> = [
  ...
  { value: 'behavior', label: 'Rude or unprofessional' },
  { value: 'fraud', label: 'Fraud or overcharging' },
...
export function ReportDialog({ open, tripCode, personName, onClose, onReported }: {
  ...
  async function submit() {
    ...
    try {
      ...
      toast.success('Report sent. Our safety team will review it.');
      onReported();
      onClose();
    } catch (thrown) {
      toast.error(getApiErrorMessage(thrown, 'Could not send the report.'));
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <Dialog
      ...
      description="Reports are private. The person you report is not told who sent it."
      onClose={onClose}
      footer={<>
        <Button variant="secondary" onClick={onClose}>Cancel</Button>
        <Button variant="danger" loading={busy} onClick={() => void submit()}>Send report</Button>
      </>}
    >
      ...
      <p className="text-xs text-ink-500">In danger right now? Call 999.</p>
    </Dialog>
  );
}
