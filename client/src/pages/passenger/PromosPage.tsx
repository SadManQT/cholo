...
function headline(promo: AvailablePromo) {
  return promo.promoType === 'percentage' ? t('{0}% off', promo.value) : t('{0} off', formatBDT(promo.value));
}

function terms(promo: AvailablePromo) {
  return [
    promo.maxDiscount ? t('Up to {0}', formatBDT(promo.maxDiscount)) : null,
    promo.minFare ? t('Rides over {0}', formatBDT(promo.minFare)) : null,
    promo.validUntil ? t('Ends {0}', formatDate(promo.validUntil)) : t('No end date'),
  ].filter(Boolean).join(' · ');
}
...
export function PromosPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(thrown, t('Could not load promos.')));
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  async function copy(code: string) {
    try {
      ...
      toast.success(t('{0} copied. Paste it when you book.', code));
    } catch {
      toast.info(t('Your code is {0}.', code));
    }
  }
  ...
  return (
    <main className="mx-auto max-w-3xl space-y-5 p-4 md:p-6">
      <div>
        ...
        <h1 className="mt-1 text-2xl font-bold">{t('Promos')}</h1>
        <p className="text-sm text-ink-500">{t('Enter a code under “Promo code” when you choose your ride.')}</p>
      </div>
      {loading ? (
        <div className="space-y-3"><Skeleton variant="card" /><Skeleton variant="card" /></div>
      ) : error ? (
        <EmptyState title={t('Promos did not load')} hint={error} action={{ label: t('Retry'), onClick: () => void load() }} />
      ) : promos.length === 0 ? (
        <EmptyState icon={<TagIcon className="h-10 w-10" />} title={t('No promos right now')} hint={t('New offers appear here as soon as they go live.')} />
      ) : (
          ...
          {promos.map((promo, index) => (
            <li key={promo.code} className="animate-stagger-in flex overflow-hidden rounded-2xl border border-border bg-surface" style={staggerStyle(index)}>
              ...
              <div className="flex min-w-0 flex-1 flex-wrap items-center justify-between gap-3 p-4">
                ...
                <Button variant="secondary" onClick={() => void copy(promo.code)}>{t('Copy code')}</Button>
              </div>
            </li>
          ))}
        </ul>
