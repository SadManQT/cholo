...
const VEHICLE_DOCS: { type: VehicleDocType; label: string; hint: string; askExpiry?: boolean }[] = [
  { type: 'registration', label: 'Registration certificate', hint: 'BRTA registration (blue book or smart card).' },
  { type: 'fitness', label: 'Fitness certificate', hint: 'Current BRTA fitness certificate.', askExpiry: true },
  { type: 'insurance', label: 'Insurance', hint: 'Valid third-party insurance.', askExpiry: true },
  { type: 'tax_token', label: 'Tax token', hint: 'Current year tax token.', askExpiry: true },
];
...
function AddVehicleForm({ categories, onAdded, onCancel }: { categories: VehicleCategory[]; onAdded: () => void; onCancel: () => void }) {
  ...
  async function submit() {
    if (form.registrationNo.trim().length < 3) return setErrors({ registrationNo: 'Enter the registration number, e.g. DHAKA METRO GA 12-3456' });
    setBusy(true);
    setErrors({});
    try {
      ...
      toast.success('Vehicle added. Upload its documents next.');
      onAdded();
    } catch (thrown) {
      const fields = getApiFieldErrors(thrown);
      if (Object.keys(fields).length) setErrors(fields);
      else setErrors({ registrationNo: getApiErrorMessage(thrown, 'Could not add this vehicle.') });
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <Card className="space-y-4">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        ...
        <Input label="Registration number" value={form.registrationNo} error={errors.registrationNo} onChange={(event) => setForm({ ...form, registrationNo: event.target.value.toUpperCase() })} />
        <Input label="Brand (optional)" value={form.brand} placeholder="Toyota" onChange={(event) => setForm({ ...form, brand: event.target.value })} />
        <Input label="Model (optional)" value={form.model} placeholder="Axio" onChange={(event) => setForm({ ...form, model: event.target.value })} />
        <Input label="Year (optional)" inputMode="numeric" value={form.modelYear} error={errors.modelYear} onChange={(event) => setForm({ ...form, modelYear: event.target.value.replace(/\D/g, '').slice(0, 4) })} />
        <Input label="Colour (optional)" value={form.color} placeholder="White" onChange={(event) => setForm({ ...form, color: event.target.value })} />
      </div>
      <div className="flex gap-2">
        <Button loading={busy} onClick={() => void submit()}>Add vehicle</Button>
...
export function DriverVehiclesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(thrown, 'Could not load your vehicles.'));
    }
  }, []);
  ...
  async function putOnDuty(vehicle: DriverVehicle) {
    ...
    try {
      ...
      toast.success(`${vehicle.registrationNo} is now your on-duty vehicle.`);
      await load();
    } catch (thrown) {
      toast.error(getApiErrorMessage(thrown, 'Could not switch vehicles.'));
    } finally {
      setBusy(null);
    }
  }

  if (error) return <EmptyState title="Vehicles did not load" hint={error} action={{ label: 'Retry', onClick: () => void load() }} />;
    ...
    <main className="mx-auto max-w-3xl space-y-5 p-4 md:p-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-ink-500">Each vehicle is approved separately. Only one is on duty at a time.</p>
        </div>
        {!adding && <Button variant="secondary" onClick={() => setAdding(true)}>Add vehicle</Button>}
      </div>

      {adding && <AddVehicleForm categories={categories} onCancel={() => setAdding(false)} onAdded={() => { setAdding(false); void load(); }} />}

      {vehicles.length === 0 && !adding ? (
        <EmptyState icon={<CarIcon className="h-10 w-10" />} title="No vehicles yet" hint="Add the vehicle you'll drive to finish your application." action={{ label: 'Add vehicle', onClick: () => setAdding(true) }} />
      ) : (
        <div className="space-y-3">
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {vehicle.isOnDuty && <span className="rounded-full bg-cholo-700 px-2.5 py-1 text-xs font-semibold text-white">On duty</span>}
                  </div>
                  ...
                  {vehicle.verificationStatus === 'rejected' && vehicle.rejectionReason && (
                    <p className="mt-2 text-sm text-danger-600">Reviewer note: {vehicle.rejectionReason}</p>
                  )}
                </div>
                <div className="flex gap-2">
                  {vehicle.verificationStatus === 'approved' && !vehicle.isOnDuty && (
                    <Button loading={busy === vehicle.id} onClick={() => void putOnDuty(vehicle)}>Put on duty</Button>
                  )}
                  <Button variant="secondary" onClick={() => setExpanded(expanded === vehicle.id ? null : vehicle.id)}>
                    {expanded === vehicle.id ? 'Hide documents' : 'Documents'}
                  </Button>
                </div>
              </div>
