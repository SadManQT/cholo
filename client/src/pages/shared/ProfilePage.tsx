...
const PASSENGER_MENU: MenuRow[] = [
  { to: '/account/places', label: t('Saved places'), hint: t('Home, university and other shortcuts'), icon: <PinIcon /> },
  { to: '/promos', label: t('Promos'), hint: t('Codes available in your city'), icon: <TagIcon /> },
  { to: '/support', label: t('Support & disputes'), hint: t('Get help or contest a fare'), icon: <LifebuoyIcon /> },
];

const DRIVER_MENU: MenuRow[] = [
  { to: '/driver/documents', label: t('Documents'), hint: t('License, NID, photo and police clearance'), icon: <FileIcon /> },
  { to: '/driver/vehicles', label: t('Vehicles'), hint: t('Add a vehicle or choose which one is on duty'), icon: <CarIcon /> },
  { to: '/driver/withdrawals', label: t('Withdrawals'), hint: t('Payout accounts and cash-out requests'), icon: <BanknoteIcon /> },
  { to: '/driver/support', label: t('Support & disputes'), hint: t('Get help from the Cholo team'), icon: <LifebuoyIcon /> },
];
...
function EmergencyContacts() {
  ...
  async function add() {
    ...
    try {
      ...
      setAdding(false);
      toast.success(t('{0} will be alerted if you send an SOS.', created.name));
    } catch (thrown) {
      const fields = getApiFieldErrors(thrown);
      if (Object.keys(fields).length) setErrors(fields);
      else toast.error(getApiErrorMessage(thrown, t('Could not add this contact.')));
    } finally {
      setBusy(false);
    }
  }
  ...
  async function remove(contact: EmergencyContact) {
    try {
      ...
      toast.error(getApiErrorMessage(thrown, t('Could not remove this contact.')));
    }
  }
    ...
    <Card>
      <div className="flex items-start justify-between gap-3">
        <div className="flex gap-3">
          ...
          <div>
            <h2 className="font-semibold">{t('Emergency contacts')}</h2>
            <p className="text-sm text-ink-500">{t('We alert them if you send an SOS during a trip. Up to 5.')}</p>
          </div>
        </div>
        {!adding && (contacts?.length ?? 0) < 5 && <Button variant="secondary" onClick={() => setAdding(true)}>{t('Add')}</Button>}
      </div>
        ...
        <ul className="mt-3 divide-y divide-border rounded-xl border border-border">
          {contacts.map((contact) => (
            <li key={contact.id} className="flex items-center justify-between gap-3 px-3 py-2.5 text-sm">
              ...
              <button type="button" className="text-sm font-medium text-danger-600 hover:underline" onClick={() => void remove(contact)}>{t('Remove')}</button>
            </li>
          ))}
        </ul>
      )}
      {contacts?.length === 0 && !adding && <p className="mt-3 rounded-xl bg-surface-alt p-3 text-sm text-ink-500">{t('No emergency contacts yet. Add someone you trust.')}</p>}
      {adding && (
        <div className="mt-3 grid gap-3 rounded-xl border border-border p-3 md:grid-cols-3">
          <Input label={t('Name')} value={draft.name} error={errors.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} autoFocus />
          <Input variant="phone" label={t('Phone')} value={draft.phone} error={errors.phone} onChange={(event) => setDraft({ ...draft, phone: event.target.value })} />
          <Input label={t('Relationship (optional)')} placeholder={t('Mother, friend…')} value={draft.relationship} onChange={(event) => setDraft({ ...draft, relationship: event.target.value })} />
          <div className="flex gap-2 md:col-span-3">
            <Button loading={busy} onClick={() => void add()}>{t('Save contact')}</Button>
            <Button variant="secondary" onClick={() => { setAdding(false); setErrors({}); }}>{t('Cancel')}</Button>
          </div>
        </div>
      )}
    </Card>
...
export function ProfilePage({ driverMode = false, adminMode = false }: { driverMode?: boolean; adminMode?: boolean }) {
  ...
  async function saveProfile() {
    if (!form.fullName.trim()) {
      setProfileErrors({ fullName: t('Enter your full name') });
      return;
    }
    setProfileBusy(true);
    setProfileErrors({});
    try {
      ...
      if (form.preferredLanguage !== language) {
        setLanguage(form.preferredLanguage as Language);
        return;
      }
      await refreshUser();
      ...
      else toast.error(getApiErrorMessage(thrown, t('Could not update your profile.')));
    } finally {
  ...
  async function changePhoto(file: File | undefined) {
    ...
    if (!ACCEPTED_UPLOAD_TYPES.includes(file.type) || file.type === 'application/pdf' || file.size > MAX_UPLOAD_BYTES) {
      toast.error(t('Choose a JPG, PNG or WebP photo up to 5 MB.'));
      return;
    }
    setPhotoBusy(true);
    try {
      ...
      toast.error(getApiErrorMessage(thrown, t('Could not update your photo.')));
    } finally {
      setPhotoBusy(false);
    }
  }
  ...
  async function savePassword() {
    ...
    if (!password.current) errors.currentPassword = t('Enter your current password');
    if (password.next.length < 8) errors.newPassword = t('Password must be at least {0} characters', 8);
    if (password.next !== password.confirm) errors.confirm = t('Passwords do not match');
    ...
    try {
      ...
      else toast.error(getApiErrorMessage(thrown, t('Could not change your password.')));
    } finally {
  ...
  async function signOut() {
    try {
      ...
      toast.error(getApiErrorMessage(thrown, t('Could not sign out.')));
    }
  }
  ...
  const switchRow: MenuRow | null = driverMode && user.roles.includes('PASSENGER')
    ? { to: '/', label: t('Switch to riding'), hint: t('Book a ride as a passenger'), icon: <PinIcon /> }
    : !driverMode && user.roles.includes('DRIVER')
      ? { to: '/driver', label: t('Switch to driving'), hint: t('Go online and receive ride offers'), icon: <CarIcon /> }
      : !driverMode
        ? { to: '/driver/apply', label: t('Drive with Cholo'), hint: t('Earn on your own schedule'), icon: <CarIcon /> }
        : null;
  ...
  return (
    <main className="mx-auto max-w-3xl space-y-5 p-4 md:p-6">
      ...
      {menu.length > 0 && <nav aria-label={t('Account')} className="overflow-hidden rounded-2xl border border-border bg-surface">
        {menu.map((row) => (
      ...
      <Card>
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          <Input label={t('Full name')} value={form.fullName} error={profileErrors.fullName} onChange={(event) => setForm({ ...form, fullName: event.target.value })} />
          <Input label={t('Email (optional)')} inputMode="email" value={form.email} error={profileErrors.email} onChange={(event) => setForm({ ...form, email: event.target.value })} />
          <label className="text-sm font-medium md:col-span-2">{t('Preferred language')}
            <select value={form.preferredLanguage} onChange={(event) => setForm({ ...form, preferredLanguage: event.target.value as 'bn' | 'en' })} className="mt-1.5 h-11 w-full rounded-xl border border-border bg-surface px-3 md:w-1/2">
              <option value="en">{t('English')}</option><option value="bn">{t('বাংলা (Bangla)')}</option>
            </select>
          </label>
        </div>
        <Button className="mt-4" loading={profileBusy} onClick={() => void saveProfile()}>{t('Save profile')}</Button>
      </Card>

      <Card>
        <h2 className="font-semibold">{t('Change password')}</h2>
        <p className="text-sm text-ink-500">{t('Changing it signs you out on every other device.')}</p>
        <div className="mt-3 grid gap-3 md:grid-cols-3">
          <Input variant="password" label={t('Current password')} value={password.current} error={passwordErrors.currentPassword} onChange={(event) => setPassword({ ...password, current: event.target.value })} />
          <Input variant="password" label={t('New password')} value={password.next} error={passwordErrors.newPassword} onChange={(event) => setPassword({ ...password, next: event.target.value })} />
          <Input variant="password" label={t('Confirm new password')} value={password.confirm} error={passwordErrors.confirm} onChange={(event) => setPassword({ ...password, confirm: event.target.value })} />
        </div>
        <Button className="mt-4" variant="secondary" loading={passwordBusy} onClick={() => void savePassword()}>{t('Change password')}</Button>
      </Card>

      <Button variant="danger" className="w-full md:w-auto" onClick={() => void signOut()}>{t('Sign out')}</Button>
